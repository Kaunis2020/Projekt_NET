var clippy = {};
var animations = [];
const spring = ["mars", "april", "maj"];
const summer = ["juni", "juli", "augusti"];
const autumn = ["september", "oktober", "november"];
const winter = ["december", "januari", "februari"];
const words = [
    "Jag är ingen dollarsedel för att behaga alla",
    "Jag gillar inte Visual Basic!",
    "Kan Du programmera med Visual Basic???",
    "Ska vi pussas och kramas??!!",
    "Vad skönt att det är sommar!!!",
    "På sommaren brukar jag skejta i parken",
    "Gillar Du sommar??? Det gör jag!!!",
    "Tycker Du om JavaScript???",
    "Jag älskar sommar och glass!!!",
    "Kan Du skejta på skateboard???",
    "Jag brukar chatta med Bill Gates!!!",
    "På sommaren brukar jag bada i sjön!",
    "Kan Du simma med hajar????",
    "Ska vi sjunga och dansa??!!",
    "Har Du simmat med delfinerna???",
    "Tycker Du att jag pratar för mycket??!!",
    "Ibland suger livet väääldigt hårt!",
    "Jag tycker inte om vintern!!!",
    "Brukar Du åka skidor på vintern?",
    "På sommaren brukar jag åka vattenskidor!",
    "På vintern är det kallt och mörkt!!!",
    "Arbetet är ingen varg... springer inte åt skogen.",
    "Det är underbart när det är varmt och soligt!!!!",
    "En gång har jag chattat med Bruce Willis!!!",
    "Jag säger som John McClane: Yippiekieyae, mother fucker!!!",
    "Livet är underbart när underlivet är bart!!!!",
    "En stor kuk är en klen tröst i en fattig familj.",
    "Vad skall Du göra nu???!",
    "Jag gillar JavaScript!!!",
    "Jag vill sjunga och dansa!!!",
    "Vad ska Du göra på midsommar???",
    "Brukar Du grilla på sommaren?",
    "Jag älskar att grilla korv!",
    "En gång har jag chattat med Tom Cruise!",
    "Min favoritfilm är 'Die Hard' !!!",
    "Ibland är jag för pratsam!",
    "Kan Du programmera med JavaScript???",
    "Du får väl ha litet överseende med mig!",
    "Det är roligt att prata med Dig!",
    "Imorgon blir det ett mycket fint väder!",
    "På söndag åker jag till Frankrike.",
    "Jag skall resa till Barcelona!",
    "På lördag ska jag grilla med kompisarna!",
    "Nästa år ska jag plugga på universitetet!",
    "Jag ska plugga till programmerare!",
    "Mitt favoritspråk är Java!",
    "Jag gillar inte C++. C++ är trist och tråkigt!!!",
    "Det är inte måndag som är en dålig dag. Det är Ditt jobb som suger!",
    "Kan Du programmera med C#????",
    "Tycker Du om 'ASP.NET'????",
    "Jag föredrar PHP! PHP är ett fint språk!!!",
    "Jag säger som John McClane: Yippiekieyae, mother fucker!!!",
    "Jag ska uppfinna tidsmaskin och åka till framtiden!",
    "Min favoritfilm är 'Tillbaka till framtiden'",
    "Har Du ett konto på Facebook?",
    "Må en traktor arbeta, den är av järn!",
    "Jag är kompis med Mark Zuckerberg!",
    "I brist på bröd äter man limpa!!!!",
    "Arbetet dödar även hästar!",
    "Nu säger jag någon visdom... eller inte!",
    "På fritiden programmerar jag i Python!!!",
    "Oooj, vad jag snackar jäättemycket nu!!!",
    "Jag gillar inte Visual Basic!",
    "Jag är ingen dollarsedel för att behaga alla",
    "Kan Du programmera med Visual Basic???",
    "Det bästa botemedlet mot alkoholism är heroin",
    "Ska vi pussas och kramas??!!",
    "Vad skönt att det är sommar!!!",
    "På sommaren brukar jag skejta i parken",
    "Gillar Du sommar???",
    ...
    "Arbetet dödar även hästar!",
    "Nu säger jag någon visdom... eller inte!",
    "På fritiden programmerar jag i Python och Django!!!",
    "Oooj, vad jag snackar jäättemycket nu!!!"
];

function mixorder(arra1) {
...
clippy.Agent.prototype = {
    ...
    saySomething: function (dagsnamn, manadsnamn) {
        this.play('Idle1_1');
        this.speak("Idag är det " + dagsnamn + "!", 300);
        if (dagsnamn === "måndag") {
            this.speak("Fyyyyy faaan, vad jag hatar måndagar!!!", 400);
        }
        if (dagsnamn === "tisdag") {
            this.speak("På tisdagar raggar jag vackra norska brudar!!!");
        }
        if (dagsnamn === "onsdag") {
            this.speak("På onsdagar brukar jag supa ihjäl mig!!!");
        }
        if (dagsnamn === "torsdag") {
            this.speak("På torsdagar är jag redlöst berusad,", 200);
            this.speak("och har en jäääävla baksmälla.");
        }
        if (dagsnamn === "fredag") {
            this.speak("Vad skönt att det är fredag idag!", 200);
            this.play('Idle1_1');
            this.speak("Ordning och reda, löning på fredag!!!");
        }
        if (dagsnamn === "lördag") {
            this.speak("Jag älskar lördagar!", 200);
            this.speak("På lördagar latar jag mig!");
        }
        if (dagsnamn === "söndag") {
            this.speak("På söndagar brukar jag", 100);
            this.play('Idle1_1');
            this.speak("sova till klockan 12:00!!!", 200);
        }
         
        if (spring.indexOf(manadsnamn) !== -1) {
            this.play('Idle1_1');
            this.speak("Vad underbart att vintern är över nu!!!", 300);
        }
        if (summer.indexOf(manadsnamn) !== -1) {
            this.play('Idle1_1');
            this.speak("Vad härligt att det är sommar nu!!!", 300);
        }
        if (autumn.indexOf(manadsnamn) !== -1) {
            this.play('Idle1_1');
            this.speak("Vad synd att sommaren är slut!!!", 300);
        }
        if (winter.indexOf(manadsnamn) !== -1) {
            this.play('Idle1_1');
            this.speak("Vad trist och tråkigt att det är vinter nu!!!", 300);
        }
    },
    ...
    getSentence: function (amount) {
        var temp = [];
        temp = mixorder(words);
        if (amount > 0)
            return temp.slice(0, amount);
        else
            return temp.slice(amount);
    },

    playmyclippy: function () {
    ...
    shuffle: function () {
        ...
        for (var i = 0; i < animations.length; i++) {
            if (animations[i].indexOf('Hide') === -1
                    && animations[i].indexOf('Show') === -1) {
                templist.push(animations[i]);
            }
        }
        animations = [];
        animations = templist;
        templist = mixorder(animations);
        animations = templist;
    },
    ...
    get_animations: function () {
        ...
        for (var i = 0; i < animations.length; i++) {
            if (animations[i].indexOf('Hide') === -1
                    && animations[i].indexOf('Show') === -1
                    && animations[i].indexOf('GoodBye') === -1) {
                templist.push(animations[i]);
            }
        }
        animations = [];
    ...
    _setupEvents: function () {
        ...
        this._el.on('click', $.proxy(this._onOneClick, this));
